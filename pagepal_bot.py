...
TOKEN = "7406397751:AAH6QK06S-InTKsGyse_dQScF8KZt__Z914"
GOOGLE_BOOKS_API_KEY = "AIzaSyB4UUsFyIM8YEF1egfJprX_-CEgxi4_uKQ"
...
async def handle_start(update, context):
    await context.bot.send_message(chat_id=1792960067, text="Welcome to BookBot! Use /help for commands.")

async def handle_book(update, context):
    await context.bot.send_message(chat_id=1792960067, text="Please provide a book title or author to search for.")

async def handle_preview(update, context):
    await context.bot.send_message(chat_id=1792960067, text="Please provide a book title or author to preview.")

async def handle_list(update, context):
    await context.bot.send_message(chat_id=1792960067, text="Here's a list of popular books.")

async def handle_reading_list(update, context):
    await context.bot.send_message(chat_id=1792960067, text="Here's your reading list.")

async def handle_help(update, context):
    await context.bot.send_message(chat_id=1792960067, text="/start - Welcome message\n/book - Search for books\n/preview - Preview a book\n/list - List of popular books\n/reading_list - Your reading list\n/help - Help text")
...
async def handle_genre_query(update, context):
    ...
    if books:
        for book in books[:5]:  # Show 5 books
            await context.bot.send_message(chat_id=1792960067, text=format_book_details(book))
    else:
        await context.bot.send_message(chat_id=1792960067, text="No books found in this genre.")
